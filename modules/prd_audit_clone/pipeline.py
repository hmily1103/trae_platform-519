...
def _l3_render_topic(item: Dict[str, Any]) -> str:
    ...
    if any(k in text for k in ["二维码", "扫码", "未授权", "越权", "鉴权", "401", "权限", "风控"]):
        return "security_access"
    if any(k in text for k in ["成功条件", "成功标准", "验收", "日志", "埋点", "可测试", "审计", "追溯"]):
        return "acceptance_logging"
    if any(k in text for k in ["多人同时操作", "高并发", "并发", "排队", "限流", "锁", "幂等", "重复调用", "冲突裁决"]):
        return "concurrency_control"
    if any(k in text for k in ["弱网", "断网", "超时", "失败", "重试", "降级", "只描述成功路径", "失败处理"]):
        return "exception_flow"
    if any(k in text for k in ["回滚", "恢复", "退出", "切后台", "再次进入", "中途退出", "回到哪个状态", "状态清理"]):
        return "state_recovery"
    if any(k in text for k in ["边界", "最小值", "最大值", "空值", "非法值"]):
        return "boundary_rule"
    if any(k in text for k in ["字段", "数据", "返回", "错误码", "默认值", "类型", "长度", "必填"]):
        return "data_contract"
    return _issue_topic(item)
...
def _l3_title(item: Dict[str, Any]) -> str:
    ...
    if "是否允许重试" in text:
        return "失败后是否允许重试未定义"
    if "只描述成功路径" in text or "缺少失败处理" in text:
        return "失败路径与异常提示未定义"
    if "多人同时操作" in text:
...
def _l3_meeting_tag(item: Dict[str, Any]) -> str:
    ...
    if "是否允许重试" in text:
        return "重试规则没定"
    if "只描述成功路径" in text or "失败处理" in text:
        return "失败路径没定"
    if "多人同时操作" in text or "高并发" in text:
        return "并发裁决没定"
    if "日志" in text:
        return "日志要求没定"
    if "成功条件" in text or "成功标准" in text or "验收" in text:
        return "验收口径没定"
    if "回滚" in text or "恢复" in text or "中途退出" in text:
        return "状态恢复没定"
    if "最小值" in text or "最大值" in text or "边界" in text:
        return "边界规则没定"
    if topic == "data_contract":
        return "接口口径没定"
...
def _l3_fix_items(item: Dict[str, Any]) -> List[str]:
    ...
    if topic == "state_recovery":
        return [
            ...
            f"用户反馈：明确状态恢复失败时页面提示、客服口径和补救动作。",
        ]
    if topic == "acceptance_logging":
        return [
...
def _l3_scene(item: Dict[str, Any]) -> str:
    ...
    if topic == "exception_flow":
        return f"用户执行“{subject}”时一旦失败、超时或弱网，界面提示、状态回退和后续动作没有统一口径，现场会出现“像成功了但其实没成功”。"
    if topic == "concurrency_control":
        return f"多人同时触发“{subject}”时，系统没有明确裁决顺序，现场容易出现结果互相覆盖、重复执行或前后不一致。"
    if topic == "state_recovery":
        return f"用户在“{subject}”过程中退出、切后台或异常中断后，再次进入看到的状态可能和真实执行结果对不上。"
    if topic == "acceptance_logging":
...
def _l3_summary_from_cards(issue_cards: List[Dict[str, Any]], defects: List[Dict[str, Any]]) -> Tuple[str, str, List[str]]:
    ...
    for card in issue_cards[:5]:
        ...
        if title and title not in top_titles:
            top_titles.append(title)
        bullet = f"{statement}（{level}）" if statement else ""
        if bullet and bullet not in top3:
            top3.append(bullet)
        if len(top_titles) >= 3 and len(top3) >= 3:
            break
    if top_titles:
...
def _build_l1_local_report(stage3_json: Dict[str, Any]) -> str:
    ...
    title = _build_core_issue_title(top_issue) if top_issue else "关键规则缺口"
    one_liner = str(core.get("one_liner") or "当前 PRD 存在关键规则缺口，需要先澄清闭环再推进。")
    lines = []
    lines.append("# L1 管理摘要（本地生成）")
    lines.append("")
    lines.append("## 一、结论")
    lines.append(f"- 当前建议：**{status}**")
    lines.append(f"- 核心问题：{title}")
    lines.append(f"- 一句话：{one_liner}")
    lines.append(f"- 风险概览：P0 {p0} 个 / P1 {p1} 个 / P2 {p2} 个；{status_reason}")
    lines.append("")
    lines.append("## 二、需要优先修改的问题")
...
def _build_l2_issue_kind(item: Dict[str, Any]) -> str:
    ...
    topic = _issue_topic(item)
    feature = _derive_feature_name(item)
    dispatch_name_hit = any(k in name for k in ["优先级", "调度", "抢占", "插播", "恢复机制", "恢复规则"])
    dispatch_context_hit = any(k in text for k in ["优先级", "调度", "抢占", "插播", "中断", "打断", "冲突裁决", "同时触发", "生效顺序"])
    dispatch_recovery_hit = any(k in text for k in ["恢复", "续播", "重播", "补播", "重试", "回到主流程", "回到原流程", "恢复原状态"])
    dispatch_hit = dispatch_name_hit or (dispatch_context_hit and dispatch_recovery_hit)
    exception_hit = any(k in text for k in ["弱网", "断网", "超时", "失败", "降级", "兜底", "卡死", "转圈", "不可用"])
    ...
    concurrency_name_hit = any(k in name for k in ["并发", "幂等", "排队", "限流", "冲突裁决", "一致性"])
...
def _build_l2_issue_title(item: Dict[str, Any]) -> str:
    ...
    if kind == "state":
        return f"物理模式兼容与状态机终态闭环（{lv}）"
    return base + f"（{lv}）"


def _build_l2_risk_analysis(item: Dict[str, Any]) -> str:
    kind = _build_l2_issue_kind(item)
    topic = _issue_topic(item)
    feature = _derive_feature_name(item)
    if kind == "dispatch":
        target = "相关流程" if _is_rule_style_subject(feature) else f"“{feature}”"
        return f"当多个流程或任务同时发生时，如果{target}谁先执行、被打断后怎么办没有说清楚，现场就会出现前后行为不一致，用户和研发都会不知道该按哪个结果算。"
    if kind == "exception":
        target = "关键流程" if _is_rule_style_subject(feature) else f"“{feature}”"
        return f"如果{target}或它依赖的能力失败、超时或不可用时没有写清楚怎么兜底，现场就会出现“页面还在转，但谁也不知道下一步怎么办”的情况，用户会直接觉得功能不可用。"
    if kind == "concurrency":
        return "如果用户连续点击、多人同时操作，或者同一个请求被重复触发时没有统一处理规则，就可能出现重复执行、结果互相覆盖，甚至页面显示和后台真实结果对不上。"
    if kind == "state":
        return "如果中途退出、模式切换、异常中断后的回到哪里没有定义清楚，系统很容易留下脏状态。轻则下次进入时表现异常，重则出现黑屏、静显、卡死这类现场问题。"
    if topic == "qr_security":
        return "权限边界未定会让跨设备、跨账号访问缺少统一拦截口径，存在越权和合规风险。"
    if topic in ["cloud_degrade", "exception_flow", "state_recovery", "transfer_cleanup"]:
        return f"“{feature}”在中断、弱网或切换场景下缺少统一恢复口径，现场容易出现前后状态不一致。"
    return _biz_crash_text(
        str(item.get("name") or ""),
...
def _l2_has_traceable_quote(
    ...
    return _build_l2_issue_quote(item, stage1_snapshot=stage1_snapshot, prd_content=prd_content) != "【原文锚点不足，以下问题基于规则归纳】"
...
def _l2_pick_focus_issues(
    ...
    for it in issues:
        ...
        traceable_rank = 0 if _l2_has_traceable_quote(it, stage1_snapshot=stage1_snapshot, prd_content=prd_content) else 1
        detail_rank = -len(str(it.get("description") or ""))
        ranked.append((risk_rank, traceable_rank, detail_rank, it))
    ranked.sort(key=lambda x: (x[0], x[1], x[2]))
    return [it for _, _, _, it in ranked[:limit]]


def _l2_expand_kinds(top_kinds: List[str], limit: int = 3) -> List[str]:
    ordered = [k for k in top_kinds if k]
    for candidate in ["state", "dispatch", "concurrency", "exception", "security", "generic"]:
...
def _build_l2_local_report(stage1_output: Dict[str, Any], stage3_json: Dict[str, Any], prd_content: str = "") -> str:
    ...
    traceable_merged = [
        ...
        if isinstance(it, dict) and _l2_has_traceable_quote(it, stage1_snapshot=stage1_snapshot, prd_content=prd_content)
    ]
    ...
    top3 = _l2_pick_focus_issues(deduped_merged, stage1_snapshot=stage1_snapshot, prd_content=prd_content, limit=3)
    top_kinds = []
    for it in top3:
        if not isinstance(it, dict):
            continue
    ...
    for idx, it in enumerate(top3, start=1):
        ...
        quote = _build_l2_issue_quote(it, stage1_snapshot=stage1_snapshot, prd_content=prd_content)
...
def _render_stage4_markdown(stage3_json: Dict[str, Any]) -> str:
    ...
    if issue_cards:
        main_problem, one_liner_override, top3_override = _l3_summary_from_cards(issue_cards, defects_data)
    else:
    ...
    if issue_cards:
        ...
        lines.append("| :--- | :--- | :--- | :--- | :--- | :--- | :--- |")
        for card in issue_cards[:12]:
            evidence = "<br>".join([f"“{q.replace('|', ' ')}”" for q in card["quotes"]]) or "【未定位到可直接引用的 PRD 原文】"
    ...
    lines.append("")
    if defects_data:
        top_issue_cards = issue_cards[:3] if issue_cards else []
        rest_issue_cards = issue_cards[3:] if len(issue_cards) > 3 else []
        ...
        if not issue_cards:
            for i, defect in enumerate(_pick_top_defects(defects_data, limit=3), start=1):
                ...
                lines.append(f"- **PRD 原文**：{_issue_quote(defect) or '【未定位到可直接引用的 PRD 原文】'}")
