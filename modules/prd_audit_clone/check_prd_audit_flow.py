...
def test_pick_llm_config_path():
    """_pick_llm_config_path：优先 prd_audit/llm_config.json"""
    from modules.prd_audit.views import _pick_llm_config_path, STORAGE_DIR
    path = _pick_llm_config_path()
    prd_audit_config = os.path.join(STORAGE_DIR, "llm_config.json")
    if os.path.isfile(prd_audit_config):
        assert path == prd_audit_config, "应优先 prd_audit: %s" % path
    else:
        assert "llm_config.json" in path and ("test_case" in path or "prd_audit" in path), path
