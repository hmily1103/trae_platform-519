...
DEFAULT_LINE_BUSINESS_HINTS = [
    "功能", "模块", "入口", "页面", "流程", "状态", "切换", "恢复",
    "配置", "规则", "权限", "异常", "失败", "重试", "上传", "下载",
]
...
def _derive_outline_l0(stage1: Dict[str, Any], model: Dict[str, Any]) -> str:
    ...
    key_rule = next((r for r in rules if any(k in r for k in ["上传", "保存", "权限", "开关", "处理", "同步", "提交"])), "")
    parts = []
    if feature:
...
def _state_keywords(state_name: str) -> List[str]:
    ...
    mapping = {
        "关闭": ["关闭", "不录制", "关闭开关", "关闭状态"],
        "开启": ["开启", "自动开启", "打开开关", "开始任务"],
        "处理中": ["处理中", "执行中", "进行中", "红点", "状态"],
        "保存": ["保存", "持久化", "生成结果", "落盘"],
        "上传": ["上传", "云端", "待上传", "上传成功"],
        "列表": ["列表", "记录", "二维码", "扫码", "小程序", "获取", "终端"],
    }
    ...
    return {
        ...
        "packs": {
            "generic": {
                ...
                "scheduling_extract": {
                    "mode_terms": ["模式", "状态", "配置", "前台", "后台", "在线", "离线"],
                    "scene_terms": ["场景", "页面", "入口", "详情页", "列表页", "当前环境"],
                    "constraint_terms": ["有且只能", "仅", "不可", "禁止", "必须"],
                    "actor_terms": ["用户", "系统", "服务", "任务", "流程", "弹窗", "消息", "语音"],
                },
            },
            "scheduling": {
                "line_business_hints": ["优先级", "打断", "恢复", "模式", "场景", "状态", "流程", "切换", "入口", "配置", "任务", "消息"],
                "detect_system_type": {
                    "priority": ["优先级", "打断", "恢复", "抢占", "中断", ">"],
                    "display": ["展示", "画面", "屏幕", "模式", "列表", "详情"],
                },
            },
        },
    }
...
def _infer_modules_from_content(content: str) -> List[str]:
    ...
    seeds = ["录音", "上传", "下载", "分享", "列表", "详情", "配置", "权限", "审核", "同步"]
    out = []
    for s in seeds:
        if s in text:
            out.append(s)
...
def _detect_system_type(content: str, stage1: Dict[str, Any]) -> str:
    ...
    kw_display = [str(x).lower() for x in _pack_list({"x": detect}, ["x", "display"], ["展示", "画面", "屏幕"])]
...
def _extract_scheduling_rules(content: str, stage1: Dict[str, Any], keyword_pack: Dict[str, Any] = None) -> Dict[str, List[str]]:
    ...
    mode_terms = _pack_list(pack, ["scheduling_extract", "mode_terms"], ["模式", "画中画", "展示"])
    scene_terms = _pack_list(pack, ["scheduling_extract", "scene_terms"], ["场景", "大屏端", "非指定模式"])
    constraint_terms = _pack_list(pack, ["scheduling_extract", "constraint_terms"], ["有且只能", "仅", "不可", "禁止", "必须"])
    actor_terms = _pack_list(pack, ["scheduling_extract", "actor_terms"], ["播放", "弹窗", "提示", "展示"])
    for ln in lines[:300]:
...
def _default_module_gap_rule(actor: str) -> str:
    ...
    if "进入" in a or "开始" in a:
        return "【PRD未说明】该任务是否允许被再次打断"
    if "处理" in a or "执行" in a:
        return "【PRD未说明】任务被打断后是否保留进度并可恢复"
    if "展示" in a:
        return "【PRD未说明】展示被打断后是否断点续播"
    ...
    if system_type == "scheduling_system":
        ...
        for actor in actors[:8]:
            ...
            if len(out) >= 4:
                break
        if len(out) < 3:
            default_titles = ["任务进入与占用规则", "展示抢占与更新规则", "特殊模式与兼容规则", "退出恢复与异常兜底规则"]
