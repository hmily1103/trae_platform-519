...
JSON 字段要求：
- states: 所有系统状态的数组，如 ["待处理","处理中","已完成"]
- events: 所有关键触发事件的数组，如 ["用户提交申请","系统校验失败"]
...
- transitions: 状态切换规则数组，每个元素形如:
  { "from": "待处理", "event": "用户提交申请", "to": "处理中" }
- high_priority_events: 需要重点关注的高优先级事件数组（如中断/打断/优先级相关）

示例（仅供参考，实际请根据 PRD 内容填充）：
{
  "states": ["待处理","处理中","已完成"],
  "events": ["用户提交申请","系统校验失败","用户取消操作"],
  "rules": ["提交后仅在校验通过时进入处理中"],
  "data_rules": ["处理失败时需要有明确错误码"],
  ...
  "high_priority_events": ["系统校验失败","用户取消操作","重复提交"]
}
...
7. 提取流程、规则和边界时，**请务必将其改写为通顺连贯的一段话或完整的句子**。绝对禁止：
   - 机械地用顿号、分号堆砌词语（如：“上传、下载、审核、通知；前台、后台” -> 错误）
   - 直接把原文毫无连贯性的短语生硬拼接
   - 输出没有主语和谓语的残句
   必须按人类的阅读习惯，整理出清晰的逻辑。例如：
   “文件上传支持前台与后台两种触发方式，且在处理中不可重复提交；若处理中断，系统需要提示用户并允许恢复。”
...
def extract_prd_structure(prd_text: str, llm_config_path: str, timeout: int = 90, llm_config_override: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    # Stage1 有 rule-first 兜底，但过短超时会导致长期无法用上 LLM 的结构化能力（进而影响锚点与可读性）。
    # 配合 llm_client 的 (connect, read) 拆分，这里放宽 read-timeout 上限到 120s，并允许 1 次重试。
    stage1_timeout = max(20, min(int(timeout or 120), 120))
    try:
        ...
        logger.warning("extract_prd_structure failed, fallback to rule-first stage1 (timeout=%ss): %s", stage1_timeout, e)
