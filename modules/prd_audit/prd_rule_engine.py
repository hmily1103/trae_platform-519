...
{
  "module": "提交入口",
  "anchor": "L18: 用户点击“提交”后，系统直接进入处理中",
  "description": "提交入口缺少失败分支定义，例如：用户点击“提交”后如果校验失败，PRD 没写系统是停留在当前页、提示重试，还是回退到待处理态。",
  "risk_level": "P1",
  "reason": "提交动作触发 -> 校验失败 -> 页面状态无定义 -> 用户重复点击或误判成功 -> 现场操作卡住并引发重复请求。",
  "suggestion": "在“提交”流程后补充失败分支：当校验失败时，页面停留在提交入口页，展示明确失败提示，并允许用户修正后重新发起操作。"
}

示例2：
{
  "module": "参数输入框",
  "anchor": "L42: 用户输入参数值后点击确认",
  "description": "参数输入缺少边界规则，例如：PRD 没写最小值、最大值、是否允许空值或非法格式，开发和测试会各自猜口径。",
  "risk_level": "P2",
  "reason": "用户输入空值/非法值/超大值 -> 前后端校验口径不一致 -> 可能出现接口报错、结果展示异常或数据错误。",
  "suggestion": "在“参数输入”规则中补充：最小值、最大值、格式要求、是否允许空值，以及超界时的前端提示文案和后端错误码。"
}
...
def _dynamic_suggestion(module: str, defect_type: str, risk_level: str, description: str) -> str:
    ...
    if any(k in blob for k in ["同步时效", "同步延迟", "时效性", "多久同步", "列表同步"]):
        return "补充同步 SLA：明确主端完成后多少秒/分钟内同步到其他端，超时提示什么状态，并补告警和补偿机制。"
    if any(k in blob for k in ["二维码", "扫码", "鉴权", "token", "401", "未授权", "有效期"]):
        return "补充敏感资源鉴权规则：账号绑定关系、凭证有效期、越权访问返回码、失效提示和审计日志。"
    if any(k in blob for k in ["云端", "上传", "503", "待处理", "降级", "自动重试"]):
        return "补充云端异常降级方案：本地暂存、待处理标记、自动重传时机、失败提示文案和避免数据不一致的规则。"
    if any(k in blob for k in ["全局配置", "开关", "设置生效", "生效时机"]):
        return "补充配置开关的生效时机：影响当前进行中任务还是下一次任务、切换中是否影响已落地内容、页面如何提示。"
    if any(k in blob for k in ["退出", "重置", "重开", "清空", "此处规则需确认"]):
        return "补充环境重置时的统一规则：到底清空还是保留相关状态，并同步定义页面提示和恢复策略。"
    if any(k in blob for k in ["性能", "QPS", "CPU", "处理时延", "带宽", "保存期限", "容量"]):
        return "补充性能指标：处理耗时、传输时延、CPU/内存占用、并发上限和保存期限。"
    if any(k in m for k in ["入口", "链接", "资源", "页面"]) and ("安全" in t or "权限" in t):
        return "补充访问隔离与鉴权机制（资源绑定关系、会话令牌校验、访问范围限制）并记录审计日志与拦截策略。"
    if "状态" in t:
...
def run_stage2_defect_scan(stage1_output: Dict[str, Any], llm_config_path: str, timeout: int = 90, prd_text: str = "", llm_config_override: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    # Stage2 已有规则库扫描作为稳定兜底，但过短的超时会导致“长期只能规则库”。
    # Stage2 需要完整返回 JSON，且推理常见“连接快、read 慢”；配合 llm_client 的 (connect, read) 拆分，
    # 这里把 read-timeout 上限放宽到 180s，并多给一次重试（指数退避）。
    stage2_timeout = max(30, min(int(timeout or 180), 180))
    try:
        ...
        logger.warning("run_stage2_defect_scan LLM 调用失败，回退规则库结果 (timeout=%ss): %s", stage2_timeout, e)
        llm_scan_ok = False
        llm_error = str(e)
...
def run_stage2_shift_left_analysis(stage1_output: Dict[str, Any], defects: List[Dict[str, Any]], llm_config_path: str, timeout: int = 120, llm_config_override: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    引入 defects 上下文，确保左移资产能够涵盖核心的高价值洞察。
    """
    trimmed_structure = _trim_stage1_for_stage2(stage1_output or {})
    structure_json = json.dumps(trimmed_structure, ensure_ascii=False, separators=(",", ":"))
